...
RSpec.describe UserMailer, type: :mailer do
  ...
  def expiration_notice(food)
    ...
    when Date.tomorrow
      ...
      food.expiration_date.strftime('%m月%d日')
    end
  end
  ...
  describe "食材の消費期限の通知メールの送信" do
    context '消費期限日が当日の食材が存在する場合' do
      let(:mail) { UserMailer.expiration_notice(user, food_today, expiration_notice(food_today) , []) }
    ...
    context '消費期限日が明日の食材が存在する場合' do
      let(:mail) { UserMailer.expiration_notice(user, food_tomorrow, expiration_notice(food_tomorrow) , []) }
      it '正しいメール内容が送られること' do
        expect(mail.to).to eq([user.email])  # 送信先の確認
        ...
        expect(mail.body.encoded).to include('明日')  # メール本文に消費期限が含まれているか
      end
    end
    context '消費期限日が二日後の食材が存在する場合' do
      let(:mail) { UserMailer.expiration_notice(user, food_future, expiration_notice(food_future) , []) }
      it '正しいメール内容が送られること' do
        expect(mail.to).to eq([user.email])  # 送信先の確認
        expect(mail.subject).to eq("#{food_future.name}の消費期限通知")  # 件名の確認
        expect(mail.body.encoded).to include(food_future.name)  # メール本文に食材名が含まれているか
        expected_date = food_future.expiration_date.strftime('%m月%d日')
        expect(mail.body.encoded).to include(expected_date)  # メール本文に消費期限が含まれているか
      end
    end
    context 'レシピが存在している場合' do
      let(:mail) { UserMailer.expiration_notice(user, food_carrot, expiration_notice(food_carrot), recipes) }
      it 'メール本文にレシピが含まれていること' do
        expect(mail.body.encoded).to include(food_carrot.name)
    ...
    context 'レシピが存在しない場合' do
      ...
      let(:mail) { UserMailer.expiration_notice(user, food_onion, expiration_notice(food_onion), recipes)}
